...
////////////////////				////////////////////				////////////////////
	...
	if(trim($_POST['dy']) == ''){
		$errors [] = "YEAR <font color='#F1BD2D'>Campo obligatorio.</font>";
		}
	
	if(trim($_POST['dm']) == ''){
		$errors [] = "MONTH <font color='#F1BD2D'>Campo obligatorio.</font>";
		}
	
	if(trim($_POST['dd']) == ''){
		$errors [] = "DAY <font color='#F1BD2D'>Campo obligatorio.</font>";
		}
...
////////////////////				////////////////////				////////////////////
	...
	if(strlen(trim($_POST['factnum'])) == 0){
		$errors [] = "FACTURA NUMERO <font color='#F1BD2D'>Campo obligatorio.</font>";
		}
	
	elseif (strlen(trim($_POST['factnum'])) < 5){
		$errors [] = "FACTURA NUMERO <font color='#F1BD2D'>Más de 4 carácteres.</font>";
		}
		
	elseif (!preg_match('/^[^@´`\'áéíóú#$&%<>:´"·\(\)=¿?!¡\[\]\{\};,:\.\*\']+$/',$_POST['factnum'])){
		$errors [] = "FACTURA NUMERO <font color='#F1BD2D'>Caracteres no válidos.</font>";
		}
		
	elseif (!preg_match('/^[a-zA-Z,0-9_\s]+$/',$_POST['factnum'])){
		$errors [] = "FACTURA NUMERO <font color='#F1BD2D'>Solo letras o números sin acentos.</font>";
		}

	elseif (!preg_match('/^[A-Z,0-9_\s]+$/',$_POST['factnum'])){
$errors [] = "FACTURA NUMERO <font color='#F1BD2D'>Solo mayusculas, números sin acentos 0 _.</font>";
		}

    /*
	if(strlen(trim($_POST['factdate'])) == 0){
		$errors [] = "FECHA <font color='#F1BD2D'>Campo obligatorio.</font>";
		}
	
	elseif (strlen(trim($_POST['factdate'])) < 5){
		$errors [] = "FECHA <font color='#F1BD2D'>Más de 4 carácteres.</font>";
		}
		
	elseif (!preg_match('/^[^@´`\'áéíóú#$&%<>:´"·\(\)=¿?!¡\[\]\{\};,:\.\*\']+$/',$_POST['factdate'])){
		$errors [] = "FECHA <font color='#F1BD2D'>Caracteres no válidos.</font>";
		}
	elseif (!preg_match('/^[a-zA-Z,0-9\s]+$/',$_POST['factdate'])){
		$errors [] = "FECHA <font color='#F1BD2D'>Solo letras o números sin acentos.</font>";
		}
    */
	...
	if(strlen(trim($_POST['factnom'])) == 0){
		$errors [] = "RAZON SOCIAL <font color='#F1BD2D'>Campo obligatorio.</font>";
		}
	
	elseif (strlen(trim($_POST['factnom'])) < 4){
		$errors [] = "RAZON SOCIAL <font color='#F1BD2D'>Más de 3 carácteres.</font>";
		}
		
	elseif (!preg_match('/^[^@´`\'áéíóú#$&%<>:´"·\(\)=¿?!¡\[\]\{\};,:\.\*\']+$/',$_POST['factnom'])){
		$errors [] = "RAZON SOCIAL <font color='#F1BD2D'>Caracteres no válidos.</font>";
		}
		
	elseif (!preg_match('/^[a-zA-Z,0-9_\s]+$/',$_POST['factnom'])){
		$errors [] = "RAZON SOCIAL <font color='#F1BD2D'>Solo letras, números sin acentos o _.</font>";
		}

	 // VALIDO EL CAMPO factnif NIF DEL CLIENTE
	
	if(strlen(trim($_POST['factnif'])) == 0){
		$errors [] = "NIF/CIF <font color='#F1BD2D'>Campo obligatorio.</font>";
		}
	
	elseif (strlen(trim($_POST['factnif'])) < 5){
		$errors [] = "NIF/CIF <font color='#F1BD2D'>Más de 4 carácteres.</font>";
		}
		
	elseif (!preg_match('/^[^@´`\'áéíóú#$&%<>:´"·\(\)=¿?!¡\[\]\{\};,:\.\*\']+$/',$_POST['factnif'])){
		$errors [] = "NIF/CIF <font color='#F1BD2D'>Caracteres no válidos.</font>";
		}
		
	elseif (!preg_match('/^[A-Z,a-z,0-9\s]+$/',$_POST['factnif'])){
		$errors [] = "NIF/CIF <font color='#F1BD2D'>Solo mayusculas o números sin acentos.</font>";
		}

	// VALIDO EL campo factiva
	
	if($_POST['factiva'] == ''){
		$errors [] = "IMPUESTOS: <font color='#F1BD2D'>SELECCIONE EL TIPO DE IVA</font>";
	}
	...
	if($_POST['factret'] == ''){
		$errors [] = "RETENCIONES: <font color='#F1BD2D'>SELECCIONE EL TIPO DE IVA</font>";
		}
	...
	// VALIDO EL CAMPO factpvp
		...
		if(strlen(trim($_POST['factpvp1'])) == 0){
			$errors [] = "SUBTOTAL € <font color='#F1BD2D'>CAMPO OBLIGATORIO</font>";
			}
		
		elseif (!preg_match('/^[^@´`\'áéíóú#$&%<>:´"·\(\)=¿?!¡\[\]\{\};,:\*\']+$/',$_POST['factpvp1'])){
			$errors [] = "SUBTOTAL € <font color='#F1BD2D'>CARACTERES NO VALIDOS.</font>";
			}
			
		elseif (!preg_match('/^[0-9]+$/',$_POST['factpvp1'])){
			$errors [] = "SUBTOTAL € <font color='#F1BD2D'>SOLO NUMEROS</font>";
			}
	...
	// VALIDO EL CAMPO factpvptot
		...
		if(strlen(trim($_POST['factpvptot1'])) == 0){
			$errors [] = "TOTAL € <font color='#F1BD2D'>CAMPO OBLIGATORIO</font>";
			}
		
        elseif (!preg_match('/^[^@´`\'áéíóú#$&%<>:´"·\(\)=¿?!¡\[\]\{\};,:\*\']+$/',$_POST['factpvptot1'])){
                $errors [] = "TOTAL € <font color='#F1BD2D'>CARACTERES NO VALIDOS.</font>";
                }
			
		elseif (!preg_match('/^[0-9]+$/',$_POST['factpvptot1'])){
			$errors [] = "TOTAL € <font color='#F1BD2D'>SOLO NUMEROS</font>";
			}
	...
	if(strlen(trim($_POST['coment'])) == 0){
		$errors [] = "DESCRIPCIÓN <font color='#F1BD2D'>Campo obligatorio.</font>";
		}
	
	elseif (strlen(trim($_POST['coment'])) < 10){
		$errors [] = "DESCRIPCIÓN <font color='#F1BD2D'>Más de 10 carácteres.</font>";
		}
		
	elseif (strlen(trim($_POST['coment'])) > 19){

	if (!preg_match('/^[a-z A-Z 0-9 \s ,.;:\'-() áéíóúñ € \/]+$/',$_POST['coment'])){
		$errors [] = "DESCRIPCIÓN  <font color='#F1BD2D'>A escrito caracteres no permitidos. { } [ ] ¿ ? < > ¡ ! @ # ...</font>";
		}
	}
	...
	if((isset($_POST['factnum']))&&(strlen(trim($_POST['factnum'])) != 0)){
		...
		/* INICIO VERIFICO EL NUMERO DE LA FACTURA EN TODAS LAS TABLAS ACTIVAS MENOS PAPELERA STATUS */
			...
			while($yearMin<=$yearHoy){
				...
				if($countgw > 0){ $errors [] = "<font color='#F1BD2D'>YA EXISTE LA FACTURA EN ".$vnamegw." </font>"; }
		...
		if((isset($_POST['id']))&&(strlen(trim($_POST['id'])) != 0)){
			...
			//$rowsg = mysqli_fetch_assoc($qg);	
        if($countg > 0){
			$errors [] = "<font color='#F1BD2D'>YA EXISTE LA FACTURA EN ".$vnameg." </font>";
	    }
		...
		if($ingresoModif3 == 1){
			...
			if($countgp > 0){
				$errors [] = "<font color='#F1BD2D'>YA EXISTE LA FACTURA EN ".$vnamegp." </font>";
			} 
		}
		...
		if($papelera == 1){
			...
			if($countgpap > 0){
				$errors [] = "<font color='#F1BD2D'>YA EXISTE LA FACTURA EN ".$vnamegpap." </font>";
			} 
		}
